...
import { ViewFriendsURL, ViewFriendRequestsURL, SearchFriendURL, AddFriendRequestURL, RemoveFriendURL, AcceptFriendRequestURL, RemoveFriendRequestURL } from "./settings";
...
function Friends({ apiFetchFacade }) {
  ...
  function viewFriendsHandler(event) {
    ...
    apiFetchFacade()
      ...
      .catch((err) => {
        if (err.status === 500) {
          setResponse("Something unexpected went wrong, please try again later.");
        } else if (err.status === 404) {
        setResponse("You currently do not have any friends, get out there and add some!");
  }});
  };
  ...
  function DisplaySearch({search}) {
    return (
      <tr>
        ...
        <td><button onClick={(event) => addFriendHandler(search.userID)}>Add to friend</button></td>
      </tr>
    );
  }
  ...
  function DisplayFriends({friends}) {
    return (
      <tr>
        ...
        <td><button onClick={(event) => removeFriendHandler(friends.friendID)}>Remove friend</button></td>
      </tr>
    );
  }
  ...
  function DisplayFriendsRequests({friendsReq}) {
    return (
      <tr>
        ...
        <td><button onClick={(event) => acceptFriendReqHandler(friendsReq.friendID)}>Accept Friend Request</button></td>
        <td><button onClick={(event) => removeFriendReqHandler(friendsReq.friendID)}>Remove Friend Request</button></td>
      </tr>
    );
  }
  ...
  return (
    <div>
      ...
      <button onClick={viewFriendsHandler}>
      View Friends
</button>
      <button onClick={(event) => viewRequestsHandler(event)}>View Friend Requests</button>
      <p>
        Search Friend:{" "}
        <input
          type="text"
